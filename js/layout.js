...
import { NAV, roleLabel, roleColor, canAccess } from './roles.js';
...
export function renderShell(profile, activeRoute, onNavigate) {
  const items = NAV.filter((n) => n.id !== 'profile' && canAccess(n.id, profile.role));
    ...
    <div class="shell">
      <aside class="sidebar" id="sidebar">
        ...
        <div class="sidebar-foot">
          <a href="#/profile" class="user-pill" data-route="profile">
            <div class="avatar" style="background:${colorFor(name)}">${initials(name)}</div>
            <div class="info">
              <b>${esc(name)}</b>
      ...
      <div class="main">
        <header class="topbar">
          ...
          <span class="role-tag ${profile.role}" style="background:${roleColor(profile.role)}22;color:${roleColor(profile.role)}">${esc(roleLabel(profile.role))}</span>
